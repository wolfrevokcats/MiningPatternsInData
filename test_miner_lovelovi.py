...
if __name__ == "__main__":
	...
	datasets = ['chess.dat', 'mushroom.dat','retail.dat']
	for i in range(0,len(datasets)):

		out_file = open("plot/time_test_" + datasets[i], "w")

		for k in range(0,10):
			print("{}: run {}".format(datasets[i], k), end='\r')
			t1 = time.time()
			alternative_miner(datasets[i], 0.8)
			t2 = time.time() - t1
